...
from devtools.constants import ActionType, ListboxPageInsertType, TreeStateKey
...
class RightWindowFrame(tk.Frame):
    def __init__(self, 
        ...
        btn1 = ttk.Button(self.top_row_wrapper, text=Style.right_window['header']['top_row']['btn1_text'])
        btn2 = ttk.Button(self.top_row_wrapper, text=Style.right_window['header']['top_row']['btn2_text'])
        ...
        self.pack_listbox_page_insert()
    
    def pack_listbox_page_insert(self: ListboxPageInsertType):
        try:
            if self._store.current_listbox_insert:
                self._store.current_listbox_insert.pack_forget()
            match self._store.listbox_page_insert_type:
